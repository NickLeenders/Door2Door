...
w_cargo = w_components(wing_weight(1630)).w_cargo
...
def function_XCG():
    X_CG = (
                   x_tail * w_tail + x_structure * w_structure + x_propeller * w_propeller + x_wing * w_wing + x_drivetrain * w_drivetrain \
                   + x_fuel * w_fuel + x_cell * w_cell + x_payload * w_payload + x_battery * w_battery + x_tank * w_tank + x_cargo * w_cargo) / (
                   w_tail + w_structure \
...
def function_ZCG():
    X_CG = (
                   ...
                   + z_fuel * w_fuel + z_cell * w_cell + z_payload * w_payload + z_battery * w_battery + z_tank * w_tank + z_cargo * w_cargo) / (
                   w_tail + w_structure \
                   + w_propeller + w_wing + w_drivetrain + w_fuel + w_cell + w_payload + w_battery + w_tank + w_cargo)
    return (X_CG)
